...
const findUserByEmailQuery = `SELECT id FROM user WHERE email = ?`

func (cm CassandraManager) FindUserByEmail(ctx context.Context, email string) (model.User, error) {
	query := cm.session.Query(findUserByEmailQuery,
		email,
	)

	var id gocql.UUID
	if err := query.WithContext(ctx).Consistency(gocql.One).Scan(&id); err != nil {
		log.Printf("FindUserByEmail Error: %v", err)
		return model.User{}, err
	}

	return model.User{Id: id.String()}, nil
}

const findUserByUsernameQuery = `SELECT id FROM user WHERE username = ?`

func (cm CassandraManager) FindUserByUsername(ctx context.Context, username string) (model.User, error) {
	query := cm.session.Query(findUserByUsernameQuery,
		username,
	)

	var id gocql.UUID
	if err := query.WithContext(ctx).Consistency(gocql.One).Scan(&id); err != nil {
		log.Printf("FindUserByUsername Error: %v", err)
		return model.User{}, err
	}
