...
func (db *DB) actions(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	if req.Data["action"] == "list" {
		return db.listActions(ctx, req)
	} else if req.Data["action"] == "mutate" {
		return db.mutateActions(ctx, req)
	} else {
		return nil, errors.New("Invalid action received.")
	}
}

func (db *DB) listActions(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	key := userKeyspace(req.Extras["user"].Data[0], req.Extras["namespace"].Data[0])
	resKey := resourceKeyspace(key, "actions")
...
func (db *DB) mutateActions(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
