...
func (db *DB) roles(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	if req.Data["action"] == "list" {
		return db.listRoles(ctx, req)
	} else if req.Data["action"] == "mutate" {
		return db.mutateRoles(ctx, req)
	} else {
		return nil, errors.New("Invalid action received.")
	}
}

func (db *DB) listRoles(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	key := userKeyspace(req.Extras["user"].Data[0], req.Extras["namespace"].Data[0])
	resKey := resourceKeyspace(key, "roles")
...
func (db *DB) mutateRoles(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
