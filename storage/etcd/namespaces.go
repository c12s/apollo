...
func (db *DB) namespaces(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	if req.Data["action"] == "list" {
		return db.listNamespaces(ctx, req)
	} else if req.Data["action"] == "mutate" {
		return db.mutateNamespaces(ctx, req)
	} else {
		return nil, errors.New("Invalid action received.")
	}
}

func (db *DB) listNamespaces(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
	key := userKeyspace(req.Extras["user"].Data[0], req.Extras["namespace"].Data[0])
	resKey := resourceKeyspace(key, "namespaces")
...
func (db *DB) mutateNamespaces(ctx context.Context, req *aPb.AuthOpt) (*aPb.AuthResp, error) {
